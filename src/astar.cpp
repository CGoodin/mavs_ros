...
*/
std::vector<std::vector<float> > Astar::PlanPath(nav_msgs::OccupancyGrid *grid, nav_msgs::OccupancyGrid *grid_segmentation, std::vector<float> goal, std::vector<float> position) {
	if (grid->info.height<=0 || grid->info.width<=0) return path_world_;

    bool has_segmentation = false; //grid_segmentation->info.height>0 && grid_segmentation->info.width>0;
