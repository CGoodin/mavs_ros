#ifndef MAVS_ROS_ASTAR_H
#define MAVS_ROS_ASTAR_H
...
class Astar {
 public:
  ...
  void SaveMap(std::string ofname);
	...
	/// Inherited from planner base class.
	std::vector<std::vector<float> > PlanPath(nav_msgs::OccupancyGrid *grid, nav_msgs::OccupancyGrid *segmentation_grid, std::vector<float> goal, std::vector<float> position);
